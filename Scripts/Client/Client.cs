...
public partial class Client : Node
{
	...
	private const int DEFAULT_PORT = 9091;  // Port number to connect to the server
	...
	private void StartClient()
	{
		...
		var result = peer.CreateClient(DEFAULT_ADDRESS, DEFAULT_PORT);
		if (result == Error.Ok)
		{
			// If client connection is successful, set the multiplayer peer to our ENet peer
			Multiplayer.MultiplayerPeer = peer;
			GD.Print($"Client connected to server on port {DEFAULT_PORT}");
	...
	// When Ping is called it is actually being alled on the "Server" instance
